...
# Tenta configurar o Tesseract. Se falhar, o sistema continua sem o OCR de censura.
TESSERACT_PATH = r'C:\Users\Pichau\Downloads\tesseract-5.5.2\tesseract.exe'
...
def censurar_documentos(base64_image: str):
    """Detecta números longos em imagens e aplica desfoque (Blur) para privacidade."""
    try:
        ...
        # OCR para detectar textos
        d = pytesseract.image_to_data(gray, output_type=pytesseract.Output.DICT)
        detectado = False

        for i in range(len(d['text'])):
            text = d['text'][i].strip()
            # Se encontrar sequência numérica longa (provável CPF/RG)
...
def criar_item_rota(item: ItemCreate, db: Session = Depends(get_db)):
    # Geofencing: Garante que o item esteja dentro do território brasileiro
...
# --- ROTAS DE NOTIFICAÇÕES (CHATS) ---

@app.post("/api/notifications")
async def post_notification(notif: NotificationCreate, db: Session = Depends(get_db)):
    # Verifica integridade do item
    item_existente = db.query(DBItem).filter(DBItem.id == notif.item_id).first()
    if not item_existente:
        raise HTTPException(status_code=404, detail="Item referenciado não encontrado")

    nova_notificacao = DBNotification(
    ...
    db.commit()
    return {"status": "sent", "id": nova_notificacao.id}
...
def responder_notificacao(req: NotificationAction, db: Session = Depends(get_db)):
    ...
    if req.action == "accepted":
        # Criação do Chat após aprovação da resposta de segurança
        novo_chat = DBChat(
            item_id=notif.item_id,
